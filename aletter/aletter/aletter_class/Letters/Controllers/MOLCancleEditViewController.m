//
//  MOLCancleEditViewController.m
//  aletter
//
//  Created by moli-2017 on 2018/8/29.
...
@interface MOLCancleEditViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation MOLCancleEditViewController
...
- (void)viewDidLoad {
    ...
    self.nameArray = @[@"退出编辑",@"我手滑了"];
    [self setupCancleEditViewControllerUI];
    ...
    [[MOLGlobalManager shareGlobalManager].global_currentNavigationViewControl.viewControllers enumerateObjectsUsingBlock:^(__kindof UIViewController * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)request_getDataSource
{
    ...
    left_model.leftTitle = @"退出将会丢失已经添加的内容";
...
- (void)button_clickCancleEditButton:(NSString *)reason
{
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (model.modelType == MOLChooseBoxModelType_rightChooseButton) {
        if ([model.buttonTitle isEqualToString:self.nameArray.lastObject]) {
            ...
            [self button_clickCancleEditButton:model.buttonTitle];
        }
    }
}
...
#pragma mark - UI
- (void)setupCancleEditViewControllerUI
{
...
- (void)calculatorCancleEditViewControllerFrame
{
...
- (void)viewDidLayoutSubviews
{
    ...
    [self calculatorCancleEditViewControllerFrame];
}
@end
